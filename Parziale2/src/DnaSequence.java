...
public class DnaSequence extends AbstractSequence implements IBioSeq {
	...
	public String reverseComplement(String sequence){
		...
		for(String nt: nucleotides){
			switch(nt){
				case "A":
					resString = "T" + resString; //i caratteri sono aggiunti in testa, quindi viene al contrario come voglio
					break;
				case "T":
	...
	//sono costretto ad avere la forma sopra non statica per via dell'interfaccia, ma questa forma mi sembra più logica, così
	//come mi sembrerebbe logico che quella sopra si possa usare su sequenze qualunque tramite riferimento statico
	public String reverseComplement(){
		return this.reverseComplement(this.sequence);
	}
	
	// 2) computeReadingFrames --------------------------------------------
	// initializes Frames and Reading frames of this and gives this.Frames as output
	...
	// 4) getComposition --------------------------------------------------------------
	//anche qui questo non statico non mi piace, poi vediamo
	...
	//è solo logico che ci sia una chiamata di default sulla sequenza che genera il metodo non statico
	public Map<String,Integer> getComposition(){
		return this.getComposition(this.sequence, this);
	}
	...
	public void printTranslations(){
		for(String readingFrame : this.Translations.keySet()){
			...
			AbstractSequence.printFasta(curheader, curSequence);
		}
	}
	...
	public DnaSequence(String header, String sequence){
		...
		computeReadingFrames(sequence);
		translateFrames(this.Frames); 
	}	
}
