...
public class SW_Esercizio_punto2_input_Utente {		
	
	public static Double askDouble(BufferedReader lettore, String message) throws IOException {
		Double value = null;
		while(true) { //we loop the question until a valid argument is inserted
			System.out.print(message + ":\n> ");
			try{
				value = Double.valueOf(lettore.readLine());
				System.out.println();
			}
			catch(NumberFormatException e) {
				System.out.println("Il valore inserito non è riconoscibile come numero decimale, riprova");
				continue;
			}
			break; //the cicle closes automatically unless an invalid argument is inserted
		}
		return value;
	}
	
	//si potrebbe inserire un metodo che riconosca il tipo di sequenza (DNA, RNA, Amminoacidi) e si assicuri che 
	//la sequenza aderisca consistentemente a uno di quei formati
	...
	public static void main(String[] args) {
			...
			String seq1 = null; //"AAUGCCAUUGACGG";
			String seq2 = null; //"CAGCCUCGCUUAG"; 
			
			// Scoring scheme definition:  		(we initialize them at 0 for safety reasons)
			double match = 0.0; // = 1.0d;
			double mismatch = 0.0; // = -1d/3;
			double gap = 0.0; //= -4.0d/3.0;	// fixed gap penalty
			
			// we open a BufferedReader based on System.in
			BufferedReader lettore = new BufferedReader( new InputStreamReader(System.in));
			// and ask and read the two sequences
			try {
				System.out.print("Inserire la prima sequenza:\n> ");
				seq1 = lettore.readLine();
				System.out.println();
				System.out.print("Inserire la seconda sequenza:\n> ");
				seq2 = lettore.readLine();
				System.out.println();
			//the scorings are asked to the reader
				match = askDouble(lettore, "Inserire il punteggio per il matching di elementi uguali");
				mismatch = askDouble(lettore, "Inserire il punteggio per il mismatch di elementi");
				gap = askDouble(lettore, "Inserire il punteggio per l'introduzione di un gap nell'allineamento");
			}
			catch (IOException e) { //a single try/catch block is enough since we want to manage the same error at each reading
				System.out.println("System.in non disponibile\n uscita...");
				System.exit(-1);
			}
			
			//User imput is finished. Though, e do not close the reader since it doesn't occupy resources on the disk
			//and we prefer not to close System.io
			...
			if (alignmentData[0] instanceof double[][]) {
