...
// this for cycle would be a bit better memory-wise given that the for-each below uses two equivalent counters after
// compilation. An even better approach would be to put the first print outside, so that we don't make 
// strarray.lenght-1 checks we already know the value of
...
//			}
	...
	/*I believe the following implementation would be better, memory wise, as we do not need to catch the output from
	 * computeReadingFrames and translateFrames for them to be accessible, since they are already stored into the 
	 * object native members, and the variables rf and tr above are not user accessible anyway*/
//	public DnaSequence(String header, String sequence){
//		super(header, sequence);
